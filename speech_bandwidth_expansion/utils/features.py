...
def fbf_feature_extraction(signal, fs, window, step):
    ...
    feature_names = ["LPCcoef_{0:d}".format(lpcc_i) for lpcc_i in range(1, order + 2)]
    ...
    while current_position + window - 1 < number_of_samples:
        ...
        [
            feature_vector[n_lpc_feats + n_spectral_feats],
            feature_vector[n_lpc_feats + n_spectral_feats + 1],
        ] = F1F2
        [
            feature_vector[n_lpc_feats + n_spectral_feats + 2],
            feature_vector[n_lpc_feats + n_spectral_feats + 3],
        ] = BWs
        # MFCCs
        mffc_feats_end = n_total_feats
        feature_vector[n_total_feats - n_mfcc_feats : mffc_feats_end, 0] = mfcc(
            fft_mag, fbank, n_mfcc_feats
        ).copy()
...
def formants(x, order, fs):
    ...
    x_win = x * w
    x_pre = lfilter([1], [1.0, 0.63], x_win)
